...
export default function FlowChart({ tasks, llmDistribution }: FlowChartProps) {
  ...
  return (
    <div className="card">
      <span className="lbl">Workflow overview</span>
      <p style={{ fontSize: 12, color: 'var(--txt3)', marginBottom: 12 }}>Click any step to see details</p>
          ...
          return (
            <div key={t.number} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <div
                ...
                style={{
                  background: isActive ? 'var(--pr-bg)' : 'var(--bg2)',
                  border: `1px solid ${isActive ? 'var(--pr)' : 'var(--bdr)'}`,
                  borderRadius: 'var(--r)', padding: '9px 12px',
                  textAlign: 'center', minWidth: 88, flexShrink: 0,
              ...
              >
                ...
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--txt)', marginBottom: 2 }}>{t.name}</div>
                {t.llm && t.llm !== 'None' && (
              ...
              {i < tasks.length - 1 && (
                <span style={{ color: 'var(--bdr2)', fontSize: 14, flexShrink: 0 }}>→</span>
              )}
            </div>
          );
        })}
      ...
      {activeNode !== null && (() => {
        ...
        return (
          <div className="fu" style={{ background: 'var(--pr-bg)', border: '1px solid var(--bdr)', borderRadius: 'var(--r)', padding: '12px 14px', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              {t.llm && t.llm !== 'None' && (
                <span style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  ...
                  {affLink && (
                    <a href={affLink} target="_blank" rel="noopener" style={{ fontSize: 10, color: 'var(--pr-tx)', textDecoration: 'none' }}>↗</a>
                  )}
                  {verifyLink && (
                    <a href={verifyLink.pricing} target="_blank" rel="noopener" className="btn-s" style={{ fontSize: 10, padding: '2px 7px' }}>↗ Verify pricing</a>
                  )}
                </span>
              )}
              {t.platform && (
                <span className="pill" style={{ background: 'var(--bg3)', color: 'var(--txt2)', fontSize: 10 }}>🔧 {t.platform}</span>
              )}
            </div>
